...
def rounds(monkeys: list[Monkey], n: int, prints: list[int]) -> list[Monkey]:
    for i in range(n):
        ...
        if i+1 in prints:
            ...
            for idx, monkey in enumerate(monkeys):
                print(
                    f"Monkey {idx}: inspected items {monkey.inspected} times.")
...
def main() -> None:
    ...
    for monkey in monkeys:
        factor = factor * monkey.test
    for monkey in monkeys:
        monkey.factor = factor

    monkeys = rounds(monkeys, 10000, [1, 20, 1000, 10000])
